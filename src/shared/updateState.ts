...
export const REPO = 'mx1010/goblins-gold-hoard';
...
export function installerUrl(version: string, platform: string, arch: string): string {
  ...
  const file = platform === 'darwin' ? `Goblins-Gold-Hoard-${v}-mac-${arch}.dmg`
    : platform === 'win32' ? `Goblins-Gold-Hoard-${v}-win-x64-setup.exe`
    : `Goblins-Gold-Hoard-${v}-linux-x86_64.AppImage`;
  ...
  switch (status?.state) {
    ...
    case 'downloaded':
      return {
        ...
        detail: `Restart Goblin's Gold Hoard to finish updating from v${v}.`,
