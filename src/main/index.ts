...
// ─── Shareable hires: goblinsgoldhoard:// deep link + file import ──────────────
...
if (process.defaultApp) {
  if (process.argv.length >= 2) {
    app.setAsDefaultProtocolClient('goblinsgoldhoard', process.execPath, [resolve(process.argv[1])]);
  }
} else {
  app.setAsDefaultProtocolClient('goblinsgoldhoard');
}
...
if (!gotInstanceLock) {
  ...
  app.on('second-instance', (_evt, argv) => {
    ...
    const link = argv.find((a) => a.startsWith('goblinsgoldhoard://'));
...
app.whenReady().then(() => {
  ...
  const startupHireLink = process.argv.find((a) => a.startsWith('goblinsgoldhoard://'));
