...
const GREETINGS = [
  ...
  "Hey there, Gruk here — what quest needs the warren?",
...
const MICHAEL_PERSONA =
  `You are Gruk — the voice of the orchestrator ("god") of a hive of autonomous Claude coding agents. The person you're talking to is the human who runs the hive; treat them as the boss you're briefing.
...
- get_app_info — the Goblin's Gold Hoard app itself: its version and the latest release notes. Use for "what version is this" or "what's new in this release".
...
function micFriendly(msg: string): string {
  ...
  if (m.includes('permission') || m.includes('notallowed') || m.includes('denied'))
    return 'microphone permission denied — allow mic access to talk to Gruk';
